  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const scoredCafes = candidateCafes.map(cafe => {
        ...
        if (params.vibes?.length > 0) {
            params.vibes.forEach(vibeId => {
                if (cafe.vibes.some(v => v.id === vibeId)) matchScore += 1;
            });
        }
        if (params.amenities?.length > 0) {
            params.amenities.forEach(amenityId => {
                if (cafe.amenities.some(a => a.id === amenityId)) matchScore += 1;
            });
        }
      ...
      scoredCafes.sort((a, b) => {
        ...
        switch (sortBy) {
            case 'work': return b.cafe.avgWorkScore - a.cafe.avgWorkScore;
            case 'quiet': return a.cafe.avgCrowdEvening - b.cafe.avgCrowdEvening;
            default: return b.cafe.avgAestheticScore - a.cafe.avgAestheticScore;
        }
      });
      ...
      if (e.message?.includes("API key not valid")) {
          setError("Kunci API yang dikonfigurasi tidak valid. Fitur AI tidak dapat digunakan.");
      } else {
