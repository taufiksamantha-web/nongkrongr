...
export const ReportModal: React.FC<ReportModalProps> = ({ isOpen, onClose, onSubmit, cafeName }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Lapor Masalah" className="!max-w-md w-full">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        ...
                        <div className="space-y-2">
                            {reasons.map(r => (
                                <label key={r} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${reason === r ? 'border-primary bg-primary/5 text-primary font-bold shadow-sm' : 'border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800'}`}>
                                    <input 
                                        type="radio" 
                                        name="reason" 
                                        value={r} 
                    ...
                    <div>
                        ...
                        <textarea 
                            className="w-full p-4 rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 focus:ring-2 focus:ring-primary outline-none text-sm transition-shadow shadow-sm"
                            rows={3}
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <input 
                                type="number" 
                                className={`w-full p-3 rounded-xl border ${error ? 'border-red-500 focus:ring-red-500' : 'border-gray-200 dark:border-slate-700 focus:ring-primary'} bg-white dark:bg-slate-800 outline-none text-sm font-bold`}
                                placeholder="Jawaban..."
                                value={userAnswer}
