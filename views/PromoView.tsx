...
export const PromoView: React.FC<PromoViewProps> = ({ 
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-[#0F172A] pb-32 transition-colors">
            ...
            <div className="max-w-4xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500 pt-[calc(env(safe-area-inset-top)+5rem)] md:pt-[calc(env(safe-area-inset-top)+6.5rem)] px-4 md:px-8">
                {/* Header Section */}
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center text-center py-20 md:py-24 bg-white dark:bg-slate-900 rounded-[2.5rem] md:rounded-[3.5rem] border-2 border-dashed border-gray-200 dark:border-slate-800 px-8 md:px-10 shadow-sm">
                        ...
                        <h3 className="text-xl md:text-2xl font-display font-black text-gray-800 dark:text-white">Belum Ada Promo</h3>
                        <p className="text-gray-500 dark:text-slate-400 text-xs md:text-sm mt-3 max-w-[300px] leading-relaxed font-medium">
                            Belum ada kafe yang mendaftarkan promo untuk lokasi "{String(selectedCityName)}" saat ini.
                        </p>
                        <div className="mt-8 md:mt-10">
                            <Button 
                                variant="primary" 
                                className="rounded-2xl px-10 md:px-12 h-12 md:h-14 shadow-2xl shadow-orange-500/30 font-black tracking-tight" 
                    ...
                    <div className="grid grid-cols-1 gap-6 md:gap-10 pt-4">
                        {promoCafes.map((cafe) => {
                            ...
                            const timeEst = dist ? estimateTime(dist) : null;
                            return (
                                <div key={cafe.id} className="group relative">
                                    {/* Distance Badge - Adjusted for mobile */}
                                    ...
                                    <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] md:rounded-[3rem] shadow-[0_15px_40px_-12px_rgba(0,0,0,0.08)] hover:shadow-[0_30px_60px_-12px_rgba(0,0,0,0.2)] border border-gray-100 dark:border-slate-800 overflow-hidden flex flex-col md:flex-row transition-all duration-700 md:hover:-translate-y-2">
                                        {/* Cafe Image Container - Compact on mobile */}
                                        ...
                                        {/* Promos List Container - Balanced padding */}
                                        <div className="flex-1 p-5 md:p-10 flex flex-col justify-center bg-white dark:bg-slate-900">
                                            <div className="space-y-4 md:space-y-6">
                                                {cafe.promos?.map((promo: Promo) => (
                                                    <div key={promo.id} className="relative overflow-hidden bg-gray-50/50 dark:bg-slate-800/40 border-2 border-dashed border-gray-200 dark:border-slate-700/50 rounded-[2rem] md:rounded-[2.5rem] p-4 md:p-6 flex flex-col sm:flex-row justify-between items-center gap-4 md:gap-6 group/promo transition-all duration-500 hover:border-orange-500/50">
                                                        ...
                                                        {/* Right Side: Action Button - Responsive width */}
                                                        <div className="w-full sm:w-auto shrink-0">
                                                            {promo.code ? (
                                                                <button 
                                                                    onClick={() => handleCopy(promo.code!)} 
                                                                    className="w-full sm:w-auto px-6 py-3.5 md:px-8 md:py-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-xl md:rounded-2xl text-[10px] md:text-xs font-mono font-black flex items-center justify-center gap-2.5 active:scale-95 transition-all shadow-md hover:bg-orange-600 hover:text-white dark:hover:bg-orange-600"
                                                                >
                                                                ...
                                                                <Button 
                                                                    size="sm" 
                                                                    className="w-full sm:w-auto h-12 md:h-14 px-8 md:px-10 rounded-xl md:rounded-2xl shadow-lg" 
                                            ...
                                            {/* Quick Tip */}
                                            <p className="mt-6 md:mt-8 text-[9px] md:text-[10px] text-gray-400 dark:text-slate-600 font-bold uppercase tracking-[0.2em] text-center md:text-left">
                                                Tunjukkan kode saat pembayaran di kasir
                                            </p>
                                        </div>
                                    </div>
                                </div>
